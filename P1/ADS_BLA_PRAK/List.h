...
//	List(List & _List);				// Copy Operator überladen
	~List();
	void InsertFront(int _key);		// Einfügen eines Knotens am Anfang
	void InsertBack(int _key);		// Einfügen eines Knotesn am Ende
	bool getFront(int & _key);		// Entnehmen eines Knoten am Anfang
	bool getBack(int & _key);		// Entnehmen eines Knoten am Ende
	bool del(int _key);			// löschen eines Knotens [key]
	bool search(int _key);			// Suchen eines Knoten
	bool swap(int _key1, int _key2);	// Knoten in der Liste vertauschen
