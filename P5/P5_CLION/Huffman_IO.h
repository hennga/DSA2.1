...
private:
	IOSystem file;
	int pos_anz_bitvec;		// Filepointer merken f�r die Anzahl der Bit im Bit-Vector zu spreichern
...
public:
	...
	bool write_map(std::map<int, std::vector<bool>> & Code)
	{
		...
		for (std::map<int, std::vector<bool>>::iterator i = Code.begin(); i != Code.end(); i++)	// Durchlaufe alle Zeilen
		{
			...
			if (!file.write_zahl((unsigned int)i->second.size())) return false;	// Schreibe: Anzahl der Bit in der Zeile (Codel�nge)
			for (unsigned int k = 0; k < i->second.size(); k++)
			{
				if (!file.write_bit(i->second[k])) return false;	// Schreibe: Huffman-Codewort f�r den Grauwert
			}
	...
	{
		if (open_r) return false;
		// Position merken um sp�ter die Anzahl der Bit eintragen zu k�nnen
		pos_anz_bitvec = file.get_pos();
		unsigned int anz_bit = 0;
		if (!file.write_zahl(anz_bit)) return false;	// Platzhalter f�r die Anzahl der Bits
		...
		for (unsigned int k = 0; k < grauwerte.size(); k++)
		{
			...
			if (i != Code.end())
			{
				...
				{
					if (!file.write_bit(i->second[l])) return false;	// Schreibe: Huffman-Codewort f�r den Grauwert
				}
				anz_bit += i->second.size();
			}
		}
		if (!file.write_bit_flush()) return false;
		file.set_pos(pos_anz_bitvec);	// Dateiposition zur Speicherung der Gr�sse des Bit-Vektors wieder anfahren
		if (!file.write_zahl(anz_bit)) return false;	// Platzhalter f�r die Anzahl der Bits
		return true;
	}
	bool close()
...
//			file.set_pos(file_pos);							// zum Eintragen der Anzahl der geschriebenen Bits auf den zuvor reservierten Plazthalter positionieren
//			if (!file.write_zahl(anz_vec_bit)) return false;
		}
		open_w = false;
		open_r = false;
		file.close();
		...
		bool read_map(std::map<int, std::vector<bool>> & Code)
	{
		...
		for (unsigned int i = 0; i < anzahl; i++)			// Durchlaufe alle Zeilen
		{
			...
			if (!file.read_zahl(bit_anzahl)) return false;			// Lese: Anzahl der Bit in der Zeile (Codel�nge)
			for (unsigned int k = 0; k < bit_anzahl; k++)
			{
				if (!file.read_bit(b)) return false;				// Lese: Huffman-Codewort f�r den Grauwert
				tmp.push_back(b);
			}
			file.read_bit_flush();
			Code[grauwert] = tmp;
	...
	bool read_code_vektor(std::vector<bool> & bit)
	{
		...
		bool b;
		unsigned int anz_bit;
		file.read_zahl(anz_bit);
		while (file.read_bit(b) && bit.size() < anz_bit)
		{
			bit.push_back(b);
		}
		return true;
